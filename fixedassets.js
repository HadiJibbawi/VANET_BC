...
function FixedAssetViewModel() {
    ...
    self.currentFixedAsset = ko.observable({'id': ''});
    ...
    self.openFixedAssetEditModal = function (data) {
        ...
        if (!editCurrentFixedAssetForm[0].checkValidity()) {
            ...
            postData(route, fixedasset, function (result) {
                if (result){//} == '1') {
                    ...
                    let table = $('#datatable_fixedassets=s').DataTable();
                    table.ajax.reload();
                }
                else {
...
function FixedAsset(fixedasset) {
    ...
    self.group = fixedasset == undefined ? ko.observable('') : ko.observable(fixedasset.group);
    ...
    /*let viewmodel = fixedasset_view_model == undefined ? {customerTitles: ko.observableArray()} : fixedasset_view_model;
...
const fixedassetsColumns = [
    ...
    {'data': ['faGroup' , 'false']},
    {'data': ['status' , 'false']},
    {'data': ['id', 'new String(\'<button  onclick="fixedasset_view_model.openFixedAssetEditModal(\' + data + \')" class="btn btn-bdesign-primary btn-sm"> Details <span class="fa fa-share-square"> </span></button>\')']}
];
...
$(document).ready(function () {
    ...
    $('#datatable_fixedassets tbody').on('click', 'tr', function () {
       ...
       // fixedasset_view_model.currentFixedAsset(new FixedAsset(data));
        //$('#edit_FixedAsset_modal').modal('show');
        console.log(fixedasset_view_model.currentFixedAsset(new FixedAsset(data)));
    });
